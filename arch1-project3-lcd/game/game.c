...
void main() {
  ...
  int collision_detected = 0; // Initialize collision flag for game-over simulation
  ...
  collision_detected = 1; // Simulate a collision for testing
    ...
    for (int i = 0; i < NUM_ENEMIES; i++) {
        if (enemies[i].active) {
            ...
            if (abs(enemies[i].x - playerX) < (PLAYER_SIZE / 2 + ENEMY_SIZE / 2) &&
                ...
                gameOver = 1; // Game ends on collision
            }
        }
    }
}
...
void display_game_over() {
    clearScreen(COLOR_RED); // Clear the screen to red
    drawString5x7(20, 60, "GAME OVER", COLOR_BLACK, COLOR_RED); // Display "GAME OVER"
}
