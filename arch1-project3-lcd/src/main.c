...
volatile uint8_t falling_object_position = 0; // Falling tool position (0: left, 1: middle, 2: right)
volatile uint8_t dodge_count = 0; // Number of successful dodges

extern void custom_function(); // Placeholder assembly function
...
void setup_game() {
  ...
  buzzer_init();
  enableWDTInterrupts();
  P1DIR |= BIT0 | BIT6;  // LEDs (P1.0: Green, P1.6: Red) as output
  P1OUT &= ~(BIT0 | BIT6); // Turn off both LEDs
  or_sr(0x8); // Enable global interrupts (GIE)
}

void play_hyrule_theme() {
  static uint16_t tones[] = {1000, 1200, 1500, 1700, 2000, 0}; // Example tones for 8-bit melody
...
void display_start_screen() {
  ...
  drawString11x16(10, 30, "START GAME", COLOR_GREEN, COLOR_BLACK);
  drawString5x7(20, 80, "Press BTN1 to Play", COLOR_WHITE, COLOR_BLACK);
}

void display_game() {
  clearScreen(COLOR_BLACK);
  drawString5x7(10, 10, "Dodges:", COLOR_WHITE, COLOR_BLACK);
  drawChar5x7(70, 10, '0' + dodge_count, COLOR_WHITE, COLOR_BLACK);
...
void display_game_over() {
  clearScreen(COLOR_BLACK);
  drawString11x16(10, 30, "GAME OVER", COLOR_RED, COLOR_BLACK);
...
void wdt_c_handler() {
  ...
  if (frame_count == 125) { // Update every ~0.5 seconds
    if (game_state == 1) { // Update gameplay only during active play
      play_hyrule_theme(); // Continue playing music

      // Randomly change the falling object's position
      ...
      if (falling_object_position == player_position) {
        ...
        dodge_count++;   // Increment successful dodges
...
void button_interrupt_handler() {
  if (P1IFG & BIT3) { // Button 1 pressed (Start/Restart)
    ...
    if (game_state == 0 || game_state == 3) { // Start or restart game
      game_state = 1; // Switch to "Playing" state
      dodge_count = 0; // Reset dodge counter
      P1OUT &= ~(BIT0 | BIT6); // Turn off LEDs
      play_hyrule_theme(); // Restart music
    }
