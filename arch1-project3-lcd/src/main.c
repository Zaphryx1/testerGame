...
void play_hyrule_theme() {
  // Simulate 8-bit Hyrule Field Theme (simplified version)
...
void display_game() {
  ...
  drawChar5x7(80, 10, '0' + success_count, COLOR_WHITE, COLOR_BLACK);
...
int main() {
  game_setup();
  play_hyrule_theme(); // Start theme music
  while (1) {
    switch (game_state) {
      case 0: // Start screen
        ...
        if (P1IN & BIT3) { // Check for start button press
          game_state = 1;
          success_count = 0; // Reset score
        }
        break;
      case 1: // Playing
        display_game();
        if (P1IN & BIT4) { // Pause button press
          game_state = 2;
          stop_sound();
        }
        break;
      case 2: // Paused
        ...
        if (P1IN & BIT4) { // Resume button press
          game_state = 1;
          play_hyrule_theme();
        }
        break;
      case 3: // Game over
        ...
        if (P1IN & BIT3) { // Reset button press
          ...
          success_count = 0;
          play_hyrule_theme(); // Restart theme
        }
        break;
    }
  }
